...
class fsm_class:
    ...
    def __init__(self,
                 ...
                 public_name='FSM0'):
        #  Parametros fijos
        self.__inputs = input_dict # nombre de entrada y numero de pin
        self.__states = states   # lista de estados de operacion disponibles [st0,st1,...]
        self.__states_outputs = states_outputs # numero de estado y su diccionario de salida {nombre:value,...}
        self.__outputs = output_dict # {nombre:direcion de pin en GPIO,...}
        ...
        self.__actual_input = dict.fromkeys(self.__inputs,0)
        self.__actual_state = 0
        self.__actual_output = dict.fromkeys(self.__inputs,0)
    ...
    def read_inputs(self, inputs={}):
        ...
        if inputs == {}:
            for aux_fi, aux_me in self.__inputs.items():
                self.__actual_input[aux_fi] = random.randint(0,1)
        elif inputs != {}:
            for aux_fi, aux_me in self.__inputs.items():
    ...
    def update_state(self):
        ...
        if self.__states[aux_ga].get_last():
            print("soy el ultimo estado")
            print(self.__states[aux_ga].get_last())
    ...
    def get_outputs_dict(self):
        "Obtine dicionario de las direciones de los pines de salida"
        return self.__outputs
    
    #  Sets
    def set_actual_state(self, numb):
        """Fija la etapa actual del proceso"""
